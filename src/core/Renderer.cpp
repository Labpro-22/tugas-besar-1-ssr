  ...
  while (!window->ShouldClose()) {
    BeginDrawing();
    {
      ...
      BeginMode3D(camera);
      {
        ...
        // Update angles
        if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && !isMouseOverInterface()) {
...
bool Renderer::isMouseOverInterface() {
    Vector2 mousePos = GetMousePosition();
    for (auto interfaceRenderer : interfaceRenderers) {
      if (CheckCollisionPointRec(mousePos,
                                  Rectangle{(float)interfaceRenderer->posX, (float)interfaceRenderer->posY,
                                            (float)interfaceRenderer->width, (float)interfaceRenderer->height})) {
        return true;
      }
    }
    return false;
